...
import { ProductTypeSaleSumsAndProfitViewData } from 'src/app/clients/system-api/UserApiClient.gen';
import { ProductTypeSaleSumsAndProfitViewService } from 'src/app/services/product-type-sale-sums-and-profit-view';
...
@Component({
  ...
  styleUrls: ['./sale-product-type.component.scss']
})
export class SaleProductTypeComponent implements OnInit {
  ...
  dataSource = new MatTableDataSource<ProductTypeSaleSumsAndProfitViewData>();
  totalItems: number
  ...
  constructor(
    private _productTypeSaleSumsAndProfitViewService:ProductTypeSaleSumsAndProfitViewService, 
  ...
  getAll(){
    this._productTypeSaleSumsAndProfitViewService.getAll(this.paginator.pageIndex, this.paginator.pageSize, this.name).subscribe((data) => {
      this.dataSource = new MatTableDataSource<ProductTypeSaleSumsAndProfitViewData>(data.items!);
  ...
  getAllForPieChart(column: string){
    this._productTypeSaleSumsAndProfitViewService.getAll(0, 9999999, "").subscribe((data) => {
